...
/**
 */
Sdk.GetTranslationToken = function () {};
...
// You can also implement any other server side API to fetch this i
...
Sdk.GetTranslationToken.prototype.getMetadata = function () {
  return {
    ...
    operationName: "GetTranslationToken",
  };
};

var getTranslationTokenRequest = new Sdk.GetTranslationToken();
...
var C1WebResourceNamespace = {
  ...
  autoRenewTranslationToken: function(token, timerIds) {
    ...
    if (typeof timerIds !== 'object' || timerIds === null || Array.isArray(timerIds)) {
        throw new Error('TimerIds parameter must be an object');
    }
  ...
  getOcLanguageCodeMapFromISO6391LanguageCode: function (ISO6391LanguageCode) {
    ...
    if (
      ISO6391LanguageCode in
      C1WebResourceNamespace.ISO6391LanguageCodeToOcLanguageCodeMap
    )
      lanCode =
        C1WebResourceNamespace.ISO6391LanguageCodeToOcLanguageCodeMap[
          ISO6391LanguageCode
  ...
  upsertC2LanguageInCRM: function (conversationId, c2Lang) {
    var data = {
      ...
      "msdyn_ocliveworkitemid@odata.bind":
        "/msdyn_ocliveworkitems(" + conversationId + ")",
    };

    if (
      C1WebResourceNamespace.dictForAllConversation[conversationId][
        "msdyn_C2_language_id"
      ] == null
    ) {
      // create record
      window.top.Xrm.WebApi.createRecord(
      ...
      // update record
      window.top.Xrm.WebApi.updateRecord(
        "msdyn_ocliveworkitemcontextitem",
        C1WebResourceNamespace.dictForAllConversation[conversationId][
          "msdyn_C2_language_id"
        ],
  ...
  initializeNewConversationInWebResource: async function (conversationConfig) {
    ...
    c1Language =
      C1WebResourceNamespace.getISO6391LanguageCodeFromOcLanguageCode(
        String(conversationConfig.c1Language)
      );
	...
	var currentWorkstream = null;

	// this logic allows us to turn off translation for selected workstreams (defined in set activateTranslationWorkstreams)
	try {
		currentWorkstream = window.top.Xrm.WebApi.retrieveMultipleRecords(
			"msdyn_ocliveworkitem",
			`?$select=_msdyn_liveworkstreamid_value&$filter=msdyn_ocliveworkitemid eq ${conversationId}`
		);
		currentWorkstream = await currentWorkstream;
		if (currentWorkstream.entities.length > 0) {
			currentWorkstream = currentWorkstream.entities[0]["_msdyn_liveworkstreamid_value"];
			var inactiveWorkstream = C1WebResourceNamespace.inactiveTranslationWorkstreams.has(currentWorkstream);
			if (inactiveWorkstream) {
				return Promise.resolve({
					keepTranslationOn: false,
				});
			}
    ...
    if (
      conversationConfig.inviteParams &&
      conversationConfig.inviteParams.inviteLocale
    )
      finalC2lang =
        C1WebResourceNamespace.getISO6391LanguageCodeFromOcLanguageCode(
          conversationConfig.inviteParams.inviteLocale
    ...
    try {
      ...
      finalC2langPromise = window.top.Xrm.WebApi.retrieveMultipleRecords(
        "msdyn_ocliveworkitemcontextitem",
        "?$select=msdyn_value,msdyn_name&$filter=_msdyn_ocliveworkitemid_value eq '" +
          conversationId +
          "' and msdyn_name eq 'msdyn_C2_language'"
      );

      finalC2langPromise = await finalC2langPromise;
      for (var i = 0; i < finalC2langPromise.entities.length; i++) {
        if (
          finalC2langPromise.entities[i]["msdyn_name"] == "msdyn_C2_language"
        ) {
          finalC2lang = finalC2langPromise.entities[i]["msdyn_value"];
          finalC2langId =
            finalC2langPromise.entities[i]["msdyn_ocliveworkitemcontextitemid"];
        }
      }
    } catch (err) {
  ...
  translateMessageInWebResource: function (translationConfig) {
    ...
    if (
      translateToC1orC2 ==
      Microsoft.Omnichannel.TranslationFramework.TranslateTo.C1
    ) {
      destLang =
        C1WebResourceNamespace.dictForAllConversation[conversationId]["C1Lang"];
    }
    // While translating for C2 as we know the C1 language so source langauge is c1's language.
    if (
      translateToC1orC2 ==
      Microsoft.Omnichannel.TranslationFramework.TranslateTo.C2
    ) {
      sourceLang =
        C1WebResourceNamespace.dictForAllConversation[conversationId]["C1Lang"];
      destLang =
        C1WebResourceNamespace.dictForAllConversation[conversationId][
          "finalC2Lang"
        ]; //it can still remain null. When we have no idea about C2 language
    }
    var response = null;
    //decides which transalation engine's api to call based on config set in initializeNewConversationInWebResource method
    if (
      C1WebResourceNamespace.dictForAllConversation[conversationId]["engine"] ==
      "azure"
    )
      response = C1WebResourceNamespace.translateMessageInternalAzure(
        conversationId,
        message["text"],
        message["sender"],
        sourceLang,
      ...
      response = C1WebResourceNamespace.translateMessageInternalGoogle(
        conversationId,
        message["text"],
        message["sender"],
        sourceLang,
    ...
    response.then((value) => {
      if (value.sourceLanguage == value.destinationLanguage) {
        value.sourceLanguage =
          C1WebResourceNamespace.dictForAllConversation[conversationId][
            "C1LangLocaleCode"
          ]; //replacing the current code with original C1 language code which came during initialization of new conversation
  ...
  translateMessageInternalAzure: async function (
    conversationId,
    message,
    messageSender,
    sourceLang,
    ...
    if (destLang == null || sourceLang == destLang)
      return {
        ...
        destinationLanguage:
          C1WebResourceNamespace.getOcLanguageCodeMapFromISO6391LanguageCode(
            String(sourceLang)
          ),
    ...
    try {
      ...
      var errorObj = {
        isError: true,
        errorCode:
          Microsoft.Omnichannel.TranslationFramework.ErrorCodes
            .TRANSLATION_FAILED,
      };
      return {
    ...
    if (
      ...
      if (
        ...
        C1WebResourceNamespace.upsertC2LanguageInCRM(
          conversationId,
          detectedLang
        );
      }
    ...
    return {
      ...
      destinationLanguage:
        C1WebResourceNamespace.getOcLanguageCodeMapFromISO6391LanguageCode(
          String(myJson[0]["translations"][0]["to"])
        ),
      errorObject: errorObj,
      sourceLanguage:
        C1WebResourceNamespace.getOcLanguageCodeMapFromISO6391LanguageCode(
